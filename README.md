...
- **Balance is an open, transparent question.** Strategy sims live or die on balance, so the engine validates itself against eight pathology detectors before any UI — and the current state (the fixed sweep clears, and the two open `FAIL`s both sit in the adaptive best-response cross-check) is documented in [`engine/README.md`](engine/README.md#balance-status-24-seed-baseline--adaptive-cross-check). Real-player data is the next input.
